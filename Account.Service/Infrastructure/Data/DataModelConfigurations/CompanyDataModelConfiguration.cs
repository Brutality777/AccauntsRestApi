...
namespace Account.Service.Infrastructure.Data.DataModelConfigurations
{
    internal class CompanyDataModelConfiguration : IEntityTypeConfiguration<CompanyDataModel>
    {
        public void Configure(EntityTypeBuilder<CompanyDataModel> builder)
        {
            ...
            builder.HasMany(x => x.Workers).WithMany(x => x.Company).UsingEntity<CompanyAccountRelationDataModel>(mg => mg.HasOne(prop => prop.Account).WithMany().HasForeignKey(x => x.AccountDataModelId),
                mg => mg.HasOne(x => x.Company).WithMany().HasForeignKey(prop => prop.CompanyDataModelId),
                mg =>
                {
